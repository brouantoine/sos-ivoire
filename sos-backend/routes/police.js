  ...
  try {
    const result = await db.query(
      `SELECT a.id, a.statut, a.mode_envoi, a.created_at,
       ...
       (SELECT latitude FROM localisations WHERE alerte_id = a.id ORDER BY timestamp DESC LIMIT 1) as latitude,
       (SELECT longitude FROM localisations WHERE alerte_id = a.id ORDER BY timestamp DESC LIMIT 1) as longitude
       FROM alertes a
    ...
    console.log('Erreur alertes police:', err.message);
    res.status(500).json({ error: err.message });
  }
});
// Résoudre une alerte
